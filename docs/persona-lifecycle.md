...
`PersonaManager` + `PersonaWorker` 构成 Sirius Pulse 的**多人格进程模型**。主进程负责扫描、创建、启停和监控；子进程负责加载配置、运行引擎、维护心跳。

## 为什么需要它

v1.0 之前，Sirius Pulse 是单 workspace 单会话模式。当用户需要同时运行多个不同角色（比如"月白"和" Sirius"）时，必须手动启动多个进程并管理端口冲突。v1.0 引入多人格架构后：
...
子进程（python -m sirius_pulse.persona_worker --config {dir}）
    │
    └── PersonaWorker
...
| **启动人格** | `start_persona(name)` | `subprocess.Popen` 启动 `python -m sirius_pulse.persona_worker --config {dir}` |
