...
- 供应商、Key 池、模型选择与采样参数都由外部 AMKR 承担，不要再往 `sirius_pulse/providers/` 里加厂商实现。

## 架构约束

- 编排逻辑集中在 `sirius_pulse/core/`，避免混入 provider 细节。
- `sirius_pulse/models/models.py` 是会话与 transcript 契约的唯一事实来源。
- 架构细节统一参考 `docs/guide/architecture-overview.md`。

## 构建与测试

- 可编辑安装：`python -m pip install -e .`
- 安装测试依赖：`python -m pip install -e .[test]`
- 通过脚本运行 CLI：`sirius-pulse run`
- 通过模块入口运行：`python main.py run`
- 运行测试：`pytest -q`
...
- 不要在 engine 层增加任何模型或厂商依赖：调用时只传 AMKR 任务名，模型与采样参数由 AMKR 的任务定义决定。
- **新功能文档**：实现新功能后，不生成额外的使用方法/快速启动 markdown 文档，除非用户明确提及。应将功能文档集中在现有的对应位置（如 `docs/guide/architecture-overview.md`、`docs/reference/provider-config.md` 等），或在用户主动要求时才生成专门的指南。
