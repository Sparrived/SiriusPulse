...
- **自由时间：她自己起头，不等别人先说话**：意图不能是唯一入口，否则整件事闭环——每条意图都得来自一次回复，她就永远无法自己开始任何事，群一安静就永久沉默。因此「什么都没惦记」且「很久没人找她」时，心跳会给她一段不带任何素材的自由时间（`free_time_interval_seconds`，默认 3 小时，设 0 关闭），提示词只说「接下来这段时间是你自己的」，她可以自己起个头，也可以照旧回答「什么也不做」。这是唯一一条从零开始的路径，也刻意让「空闲」成为触发器——与意图路径不冲突：意图的紧急度不该被等待抬高，而只能因别人说话才能开始做事的人格谈不上自主。自由时间有独立节奏，且**拒绝同样消耗掉这次机会**，否则每个心跳都会白烧一次调用；首次启动会把计时器置为当下，不会因状态为空（`_parse_time("")` 读作 1970）而在每次重启后立刻白送一段。
