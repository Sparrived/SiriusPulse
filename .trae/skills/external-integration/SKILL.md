...
description: "当需要让外部项目正确接入 Sirius Pulse 时使用，覆盖 Python API 调用、配置组织和安全实践。关键词：外部接入、库调用、provider 配置、多人格。"
---
...
4. `sirius_pulse/__init__.py`（顶层公开 API 清单）
5. `sirius_pulse/persona_manager.py`
6. `sirius_pulse/persona_worker.py`
7. `sirius_pulse/platforms/runtime.py`
8. `sirius_pulse/core/emotional_engine.py`
9. `sirius_pulse/core/engine_core.py`
10. `sirius_pulse/core/cognition.py`
11. `sirius_pulse/core/prompt_factory.py`
12. `sirius_pulse/config/models.py`
13. `sirius_pulse/config/manager.py`
14. `sirius_pulse/config/helpers.py`
15. `sirius_pulse/models/models.py`
16. `sirius_pulse/models/persona.py`
17. `sirius_pulse/memory/user/simple.py`
18. `sirius_pulse/skills/security.py`
19. `sirius_pulse/providers/routing.py`
20. `sirius_pulse/providers/base.py`
21. `sirius_pulse/session/store.py`
22. `sirius_pulse/utils/layout.py`
23. `sirius_pulse/webui/auth.py`（JWT 认证）
24. `main.py`
...
- **v1.1.0**：`EmotionalGroupChatEngine` 是唯一默认引擎，位于 `sirius_pulse/core/emotional_engine.py`。
- 外部系统接入时，**优先从 `sirius_pulse` 顶层导入**（`from sirius_pulse import EmotionalGroupChatEngine, Message, SessionConfig`）。
...
- 引擎自动将 token 记录持久化至 `{work_path}/token_usage.db`（SQLite）。使用 `TokenUsageStore` + `sirius_pulse.token.analytics` 进行跨会话分析。
