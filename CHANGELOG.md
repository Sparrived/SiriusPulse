...
- **夜间静默期：可以做事，但不能发出去**：本地（中国）时间 23:00 到次日 08:00 之间不投递任何主动分享。夜里发出的消息对方最早第二天才读到，多半反感多于惊喜，因此静默期只拦**投递**、不拦**做事**——她照常可以读、可以写、可以想。被拦下的意图**保持待发且不会被写成已说**，天亮后原样发送，一个字不改；也**不消耗** `share_cooldown_seconds`，否则每晚的拦截会把第二天的节奏一并吃掉。判定固定在 UTC+8（中国无夏令时），不依赖系统时区。
- **宵禁期间最多只留一条待发消息**：由于天亮后的清空受 `share_cooldown_seconds` 限制，夜间积压多少条就会摊到多少个钟头——积压 8 条时，23:00 写的那条可能到次日 15:00 才说出口，「今天的晚霞」到那时早已不是今天的事。因此 `intend_share` 在静默期内只接受一次成功登记，第二条会被拒（她也照样可以继续想、继续做事）。上限限的是**积压**而非「一天一条」：唯一那条发出、队列清空后立即可再记。
