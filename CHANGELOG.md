...
- **自主行为从未真正产出过任何东西**：`autonomy` 心跳每 900 秒评估一次意图，`autonomy_generate` 却从未登记进 AMKR 的任务表。这个任务名是**作为 `model` 字段发出去**的，AMKR 于是拿它去找同名模型，每个回合都在第一次调用上回 404（`模型 autonomy_generate 未配置`）。线上累计 710 条该 404，对应 142 次心跳，两天里自主行为一次都没成功（`memory/autonomy/` 始终不存在）。登记 `autonomy_generate` 并让运维在 AMKR 侧绑定模型后，**线上立刻出现了第一次真正的自主产出**：09-25 19:30（UTC）的回合留下 246 字正文、写入 `memory/autonomy/episodes.json`，并落了一条 `[内心] 我自己的时间：这个点还没睡，翻了翻自己的一点点源码喵。`，还自己记下一个待想的问题（`intentions.json` 里的 nascent 意图）。代码侧同时新增守护测试：扫描全量 `task_name=` 默认值，确保代码里发出的每个任务名都已登记——两边一旦再次分叉，测试立刻失败。**登记只是第一步**：框架只创建任务名，模型必须由运维在 AMKR 面板里绑定，否则新建的任务模型为空、调用照样 404；运维页因此新增 `unbound`（已登记但未绑模型）一栏，注册新建任务时日志也会提醒去绑模型——原先的巡检只比对任务**名字**，会把这个状态显示成一切正常。
- **自主回合撞到工具轮次上限时会一个字都不留下**：工具循环的正文取自**最后一轮**，而最后一轮常常仍是工具调用——模型把全部轮次花在侦察上（翻目录、读连载、查记忆），带回来的 `clean_text` 便是空的。此时"她刚读了一堆东西"在 `run_tick` 眼里等于"没产出"，被当成拒绝处理：Episode 不写、意图不 resolve。线上 09-25 18:28 那次回合就是这样烧完 30 轮、30 次调用**每次都是 0 个字**；同一天 19:30 的回合只在第 16 轮自然收口，就留下了 246 字——所以这不是必现故障，而是取决于模型把最后一轮用在说话还是用再调一次工具上，这正是不该由运气决定的事。聊天通道早已用"补一轮只说话的收尾"解决过同一问题，现在自主回合与定时任务回合也走这一步：撞上限后**摘掉工具**（`enable_tools=False`）再要一轮正文，不再指望模型自觉。这一轮只在真撞上限时发生，代价有界（多一次调用）；失败只记日志，退回到最后一轮可能夹带的文字。
- **自由时间回合失败会以心跳频率无限重试**：`_run_free_time` 原本在回合**成功之后**才写 `last_free_time_at`，于是回合抛异常时这个"上次提供时间"永远停在过期状态，下一个心跳立刻再提供一次。自主行为死于 404 的那两天，900 秒的心跳把同一次失败重试了 142 次，正是这个形状。现在这次机会在回合**开始**时就记下：失败不等于没发生过，宁可少做一次也不每个心跳重试一遍；异常同时就地记一条带堆栈的日志，指明是哪一段自主工作出的问题。
- **后台任务失败从不写进人格日志**：`BackgroundTaskSpec.run_loop()` 捕获异常后调 `logger.exception("Background task '%s' failed", ...)`，该 logger 名为 `sirius_pulse.extension_runtime`，而它不匹配 `PERSONA_LOGGER_PREFIXES` 中的任何前缀——人格日志的文件处理器是个前缀白名单，于是这条最该被看见的失败**只落在 `docker logs`**。自主行为整整两天什么都没做，而 `persona.log` 里连一行痕迹都没有（119 条失败全部写去了容器日志）。现在 `extension_runtime` 与其余漏在名单外的模块 logger（`adapters.`、`config.`、`utils.`、`persona_config`、`sirius.tools.markdown_image` 等）都补进前缀表，并加守护测试：用 AST 收集全量 `logging.getLogger(...)` 名称，任何一个落不进任何日志文件的 logger 都会让测试失败。
- **运维页的面板连到了另一台机器上的 AMKR**：面板 iframe 此前直接用 `amkr_public_url` 拼地址，而该字段在生产上被填成了 `https://amkr.sparrived.xyz`——那是**别的机器**上的一个 AMKR 实例。判别依据很明确：用本机 AMKR 的本地 key 打它得 401，而它的供应商 key 在本机得 401；两边 `/health` 的 `local_api_key_fingerprint` 不同（本机 `dfbbdda960a9` / 远程 `b73926980b3f`），模型集也不同（12 个 vs 15 个），本机 AMKR 的日志里更是查不到任何面板请求。根因是回环地址不能下发给浏览器——`amkr_base_url` 是 `http://127.0.0.1:28881`，而回环在**用户浏览器**里指向用户自己的机器——加上 AMKR **不发 CORS 头**、面板必须与取数接口同源，于是唯一处处可用的地址是本框架自己源上的一条路径。现在 WebUI 直接在同源路径 `/amkr/` 上反代 AMKR（新增 `sirius_pulse/webui/amkr_proxy.py`），`AmkrSettings.browser_base_url` 在 `amkr_base_url` 为回环（`127.0.0.1` / `localhost` / `::1`）且未配 `amkr_public_url` 时返回相对路径 `/amkr`，面板地址因此形如 `/amkr/ui/panel.html#k=…`，同机部署不必再给 AMKR 单独配域名或证书。反代**不注入密钥**，透传浏览器带来的 `Authorization`：面板从 URL fragment 取自己的面板 key，天然免输入，且只是一把只对本空间有效的受限凭据；若在此注入 `amkr_local_api_key`，就等于开出一条无需本框架认证即可增删供应商与 Key 池的同源路径。因此这条免 JWT 的路径只放行面板用得到的 `/ui/*`、`/health` 与 `/api/tasks`，其余（`/api/providers`、`/api/logs`、`/docs` 等）一律 403，且闸门比对**解码后**的路径——否则 `/amkr/ui/%2e%2e/api/providers` 会被还原成 `/api/providers` 而绕过白名单。
...
- **人格自主行为**：新增内置 `autonomy` Tool，让每个人格拥有一段属于自己的时间。它不向模型暴露入口，只注册一个慢速心跳；心跳本身不是闹钟，职责是「看看她心里还惦记着什么」，因此大多数时候的结论就是什么都不做。限制她的是**意图**而不是计数器：没有每日配额，也没有调用冷却，节奏上限就是心跳间隔本身。动机只在她**真实思考的回合里**形成，记为 `{persona}/memory/intentions.json` 里的持久意图，跨心跳存在——心跳只读意图，绝不扫描群聊记录找素材：她拿不到「已经处理过什么」，从聊天里捡素材会把她在正常回复中刚刚读过、查过的东西又排一遍队（例如有人发了链接，她在回复时已用 `web_lookup` 读过，心跳再把那条消息变成意图就只是让她重读一遍），而且这样记下的动因只能是一句常量。意图只有两种归宿，都是同一份数据的不同 `resolution`：`do`（自己去做点什么）与 `tell`（把某件事说给某个人听，**受众是意图的一部分**）。产出写入 `{persona}/memory/autonomy/episodes.json`，并作为 `scope=persona` 的记忆单元进入既有记忆，让她以后**知道**自己做过这件事。为了避免同一件始终做不完的事在每个心跳上各烧一次模型调用，单条意图最多尝试 3 次。
- **自由时间：她自己起头，不等别人先说话**：意图不能是唯一入口，否则整件事闭环——每条意图都得来自一次回复，她就永远无法自己开始任何事，群一安静就永久沉默。因此「什么都没惦记」且「很久没人找她」时，心跳会给她一段不带任何素材的自由时间（`free_time_interval_seconds`，默认 1 小时，设 0 关闭），提示词只说「接下来这段时间是你自己的」，她可以自己起个头，也可以照旧回答「什么也不做」。这是唯一一条从零开始的路径，也刻意让「空闲」成为触发器——与意图路径不冲突：意图的紧急度不该被等待抬高，而只能因别人说话才能开始做事的人格谈不上自主。自由时间有独立节奏，且**拒绝同样消耗掉这次机会**，否则每个心跳都会白烧一次调用；首次启动会把计时器置为当下，不会因状态为空（`_parse_time("")` 读作 1970）而在每次重启后立刻白送一段。
- **夜间静默期：可以做事，但不能发出去**：本地（中国）时间 23:00 到次日 08:00 之间不投递任何主动分享。夜里发出的消息对方最早第二天才读到，多半反感多于惊喜，因此静默期只拦**投递**、不拦**做事**——她照常可以读、可以写、可以想，夜里想记多少条都行。被拦下的意图**保持待发且不会被写成已说**，天亮后原样发送，一个字不改；也**不消耗** `share_cooldown_seconds`，否则每晚的拦截会把第二天的节奏一并吃掉。天亮时**不会轰炸**：积压多少条就被 `share_cooldown_seconds` 摊到多少个钟头，08:00 只放出第一条，其余按冷却一条条跟上。判定固定在 UTC+8（中国无夏令时），不依赖系统时区。
- **`intend_pursue` Tool**：与 `intend_share` 对称的 `do` 半边，让她判断「这件事值得回头自己弄明白」时登记 `what` / `why` / `kind` / `urgency`，只登记、不立刻去做，留给心跳在合适的时机推进。动因由她自己写，因此 `why` 是真实理由而不是常量。它在自主回合内**同样豁免**（`allowed_when_self_initiated` 为真）：自由时间是她唯一能自己起头的时刻，若那时不能把新念头记下来，每开一条线索都会在回合结束时断掉。防止链条失控的是意图仍要各自过闸门与尝试上限，而不是这个开关。
- **`intend_share` Tool**：对模型可见的自主相关 Tool，只**登记**她想说的话与由她自己指定的受众，绝不立即发送；可带 `intention_id` 给一条「还没想好说给谁」的旧意图补上受众，而不是重复登记。投递经既有主动消息管线（`dispatch_proactive_message`），因此白名单、投递确认与 `event_id` 幂等都由框架保证，一次投递即标记 `shared_at`，不会重复说第二遍。
- **自主行为 WebUI 页面**：新增「分析 → 自主行为」页（`GET /api/persona/autonomy`，只读），列出她当前惦记的意图（含 `resolution` / `kind` / `urgency` / 起因 `why` / 尝试次数）与自己做过的事（`episodes` 时间线）。之所以不提供写接口：从后台替她行动会让她变成「可配置」而不是「自主」。`limit` 可限制返回条数（1–200，默认 100）。
- **引擎事件桥**：新增 `sirius_pulse/webui/event_bridge.py`，订阅每个人格引擎的 `SessionEventBus` 并按人格广播给 WebSocket 客户端。此前 WebUI 的 `/ws/events` 只传 `data/` 的文件变更通知，事件总线**从未有过订阅者**，因此 `agent_turn_updated` 被 emit 进空总线、并在 `emit_event` 的 `subscriber_count <= 0` 检查处提前返回——人格自主行动在 WebUI 上没有任何实时痕迹。桥按引擎身份增量维护订阅，因此 run 模式下 WebUI 先启动、人格 worker 后挂载，以及运行期重建引擎（会换掉事件总线对象）都能跟上。`agent_turn_updated` 带 `data.origin` 区分来源，`self_initiated` 即她自己发起的回合。
