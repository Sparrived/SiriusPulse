...
- **`intend_pursue` Tool**：与 `intend_share` 对称的 `do` 半边，让她在真实对话里判断「这件事值得回头自己弄明白」时登记 `what` / `why` / `kind` / `urgency`，只登记、不立刻去做，留给心跳在合适的时机推进。动因由她自己写，因此 `why` 是真实理由而不是常量。自主回合内**不**豁免（`allowed_when_self_initiated` 为假）：推进一件事时不该顺手登记下一件，否则每个回合都会喂养下一个回合。
