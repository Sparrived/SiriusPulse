...
from sirius_chat.skills.sticker.models import StickerRecord, RECOMMENDED_TAGS
...
- 用中文，每个标签 2-4 个字
- 优先从以下推荐标签中选取，如不合适可自行补充：{recommended_tags}
- 只输出 JSON 对象，不要输出 markdown 代码块，不要输出任何其他文字
- 格式：{{"tags": ["标签1", "标签2", ...]}}"""

_SCENE_GENERALIZE_PROMPT = """你是一个表情包使用场景分析专家。根据以下信息，概括这个表情包适合使用的场景。
...
4. 不要输出标签列表，输出完整的场景描述段落
5. 只输出场景描述本身，不要输出任何前缀、标题或markdown格式

示例输出：
适合在朋友间互相抱怨工作压力、表达疲惫不想动、午睡犯困时使用。也适合在轻松闲聊中表达慵懒、卖萌或无奈的情绪，带有可爱和放松的氛围。在讨论摸鱼、摆烂、不想上班等话题时也能自然融入。"""

_SCENE_GENERALIZE_THRESHOLD = 8
_SCENE_GENERALIZE_MAX = 3
...
class StickerLearner:
    ...
    async def _maybe_generalize_scene(self, sticker_id: str) -> None:
        ...
        触发条件：该 sticker_id 的总记录数 >= _SCENE_GENERALIZE_THRESHOLD 且
        已概括次数 < _SCENE_GENERALIZE_MAX。
        """
        records = [
            r for r in self._indexer.list_all() if r.sticker_id == sticker_id
        ]
        if not records:
            return

        first = records[0]
        if first.scene_generalize_count >= _SCENE_GENERALIZE_MAX:
            return

        # 用 record 数量作为观察次数（不重复计数）
        observation_count = len(records)
        already_generalized = first.scene_generalize_count

        # 每累积 _SCENE_GENERALIZE_THRESHOLD 次观察触发一次概括
        required = _SCENE_GENERALIZE_THRESHOLD * (already_generalized + 1)
        if observation_count < required:
            return

        logger.info(
            "表情包场景概括触发: sticker_id=%s 观察次数=%d 已概括=%d",
            sticker_id,
            observation_count,
            already_generalized,
        )
        await self._generate_scene_summary(sticker_id, records)
    ...
    async def _generate_scene_summary(
        ...
        first = records[0]
        usage_contexts = "\n".join(
            f"  - {r.usage_context}" for r in records if r.usage_context
        )
        tags = list({t for r in records for t in r.tags})

        previous_summary_section = ""
        if first.scene_summary:
            previous_summary_section = (
                f"之前的场景概括：\n{first.scene_summary}\n\n"
                "请在之前概括的基础上，结合新的使用情境进行扩展和修正。"
            )

        prompt = _SCENE_GENERALIZE_PROMPT.format(
            caption=first.caption or "未知",
            tags=", ".join(tags) or "无",
            usage_contexts=usage_contexts or "无",
        ...
        try:
            ...
            request = GenerationRequest(
                ...
                messages=[{"role": "user", "content": "概括这个表情包的使用场景"}],
                temperature=0.3,
                max_tokens=300,
                purpose="sticker_scene_generalize",
            )
            ...
            if self._token_callback is not None:
                try:
                    self._token_callback(
                        task_name="sticker_scene_generalize",
            ...
            logger.warning("场景概括生成失败: sticker_id=%s %s", sticker_id, exc)
