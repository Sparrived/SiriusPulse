...
function _getModelTags(modelId, providerType = '') {
  const modelSets = providerType
    ...
    : [...new Set(providers.map(p => p.platform_type).filter(Boolean))]
        .map(type => getModelDevCache(type));
  for (const models of modelSets) {
    ...
    if (found) {
      ...
      if (found.structured_output) tags.push('结构化');
...
function _mountHealthSelects() {
  ...
  _root.querySelectorAll('[data-health-idx]').forEach(container => {
    ...
    const opts = (p.models || []).map(m => ({
      value: m,
      label: m,
      tags: _getModelTags(m, p.platform_type),
    }));
...
function renderEditCard(p, i) {
  ...
  const devModels = getModelDevCache(p.platform_type);
...
async function refreshModels() {
  ...
  try {
    ...
    if (res.success) {
      ...
      await Promise.all(providerTypes.map(type => loadModelsDevForType(type, { refresh: true })));
      editingIdx = null;
      renderList();
