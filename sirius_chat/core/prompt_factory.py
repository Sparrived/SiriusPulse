...
class PromptFactory:
    ...
    @staticmethod
    async def generate_plugin_response(
        *,
        engine: Any,
        plugin_data: Any,
        plugin_name: str = "",
        ...
        将 Plugin 返回的结构化数据委托给引擎的 LLM 做风格化生成。

        Args:
            engine: EmotionalGroupChatEngine 实例
            plugin_data: Plugin 返回的结构化数据
            plugin_name: Plugin 显示名称
            system_prompt_suffix: 追加的 system prompt
            mood_hint: 情绪提示
            tone_override: 语气覆写
        ...
        provider = getattr(engine, 'provider_async', None)
        if provider is None:
        ...
        data_json = json.dumps(plugin_data, ensure_ascii=False, indent=2) if not isinstance(plugin_data, str) else plugin_data
        ...
        # 注入人格核心信息
        if persona:
            if getattr(persona, 'persona_summary', ''):
                parts.append(getattr(persona, 'persona_summary', ''))
            traits = getattr(persona, 'personality_traits', [])
            if traits:
                parts.append(f"性格：{'、'.join(traits[:3])}")

        parts.append(f"\n【指令执行结果】")
        parts.append(f"你刚刚执行了用户的指令，获得以下数据：")
        parts.append(data_json)

        parts.append(f"\n【表达要求】")
        parts.append("- 请以自然的人格风格向用户传达以上信息")
        if mood_hint:
            parts.append(f"- 当前情绪提示：{mood_hint}")
        if tone_override:
            parts.append(f"- 语气要求：{tone_override}")
        if system_prompt_suffix:
            parts.append(f"- {system_prompt_suffix}")
        ...
        try:
            ...
            return (
                plugin_data if isinstance(plugin_data, str)
                else json.dumps(plugin_data, ensure_ascii=False)
            )
        except Exception:
            return (
                plugin_data if isinstance(plugin_data, str)
