...
def _build_update_prompt(
    *,
    ...
    return (
        f"你是人物传记维护助手。请根据一个用户的最新发言，更新你对该用户的认知档案。\n\n"
        f"人格名称：{persona_name}\n\n"
        f"=== 现有的《{user_name}》档案 ===\n"
        f"短期传记：\n{old_bio or '（尚无传记）'}\n\n"
        ...
        f"=== 近期发言记录 ===\n"
        f"{msgs_text}\n\n"
        f"请综合旧档案和新发言，输出一份更新后的完整档案。注意：\n"
        f"1. 如果旧信息与新发言冲突，以新发言为准\n"
        f"2. 如果新发言没有涉及旧档案中的某条信息，保留旧信息（除非那明显是过时的）\n"
        f"3. 删除道听途说的闲聊（别人说的但经本人证实为错的信息）\n"
        f"4. 传记不超过 500 字\n"
        f"5. 锚点每条不超过 20 字，最多 5 条\n"
        f"6. 从发言中发现该用户的其他称呼（别名/外号/代称），放在 discovered_aliases 中\n\n"
        f"严格输出 JSON：\n"
        f'{{"short_bio": "浓缩传记全文（不超过500字）", '
        f'"identity_anchors": ["锚点1", ...], '
        f'"relationships": [{{"target": "对方名", "fact_hint": "事实描述"}}, ...], '
        f'"discovered_aliases": ["别名1", ...]}}'
    )
...
class BiographyManager:
    ...
    async def maybe_update_biography(
        ...
        *,
        ...
        """如果攒的消息足够多，调用一次 LLM 重写传记。

        触发条件：pending_messages >= 8 或距上次更新 >= 24h 且有新消息。
        ...
        # 触发条件
        should_update = len(card.pending_messages) >= 8
