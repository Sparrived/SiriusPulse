    ...
    container.innerHTML = `
      <div class="card">
        ...
        <div style="padding:40px;text-align:center;color:var(--text-3)">
          ...
          <div style="font-size:13px">在下方人格栏中选择要配置的人格</div>
        </div>
      </div>
      <div style="margin-top:20px" id="personaSelectBar"></div>
    `;
    renderPersonaBar($('personaSelectBar'));
    return;
  }
...
function renderPersonaBar(el) {
  const personas = store.personas || [];
  if (!personas.length) {
    el.innerHTML = '<div style="color:var(--text-3);padding:12px;text-align:center">暂无人格</div>';
    return;
  }
  ...
  el.innerHTML = `
    <div class="card">
      <div class="card-header">
        <div class="card-title">人格选择</div>
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:8px;padding:12px">
        ${personas.map(p => `
          <button class="btn btn-sm${p.name === store.currentPersona ? ' btn-primary' : ''}" data-name="${p.name}">
  ...
  el.querySelectorAll('button[data-name]').forEach(btn => {
    btn.addEventListener('click', () => {
      selectPersona(btn.dataset.name);
      navTo('persona', btn.dataset.name);
    });
  });
}
