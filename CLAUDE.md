...
- **`sirius_pulse/memory/`** — Layered memory: basic (retains the raw tail until a checkpoint memory unit covers it; `DEFAULT_BASIC_MEMORY_HARD_LIMIT` is only a RAM safety ceiling, and the chat prompt injects just the most recent `DEFAULT_BASIC_MEMORY_HISTORY_TOKEN_BUDGET` tokens of that window), diary (LLM summaries + ChromaDB vectors), semantic (vector search at group/user/global levels), biography (cross-session character profiles).
